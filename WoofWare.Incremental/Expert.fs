...
module internal Expert =
    ...
    let invariantAboutNumInvalidChildren (t : Expert<'a>) (isNecessary : bool) : unit =
        let {
                Children = children
                NumChildren = numChildren
                NumInvalidChildren = numInvalidChildren
            } =
            t
    ...
    let makeStale (t : Expert<'a>) : StaleResult =
        if t.ForceStale then
            StaleResult.AlreadyStale
        else
            t.ForceStale <- true
            StaleResult.Ok

    let incrInvalidChildren t =
        t.NumInvalidChildren <- t.NumInvalidChildren + 1

    let decrInvalidChildren t =
        t.NumInvalidChildren <- t.NumInvalidChildren - 1
