...
module internal Kind =
    ...
    let invariant<'a> (inv : 'a -> unit) (t : Kind<'a>) : unit =
        ...
        | Kind.ArrayFold arrayFold ->
            { new ArrayFoldEval<_, _> with
                member _.Eval arrayFold =
                    ArrayFold.invariant ignore inv arrayFold |> FakeUnit.ofUnit
            }
            |> arrayFold.Apply
            |> FakeUnit.toUnit
        ...
        | Kind.IfTestChange (ifThenElse, _) ->
            { new IfThenElseEval<_> with
                member _.Eval iTE =
                    IfThenElse.invariant ignore iTE |> FakeUnit.ofUnit
            }
            |> ifThenElse.Apply
            |> FakeUnit.toUnit
        ...
        | Kind.JoinLhsChange (join, _) ->
            { new JoinEval<_> with
                member _.Eval join =
                    Join.invariant ignore join |> FakeUnit.ofUnit
            }
            |> join.Apply
            |> FakeUnit.toUnit
        ...
        | Kind.StepFunction step_function_node -> StepFunctionNode.invariant inv step_function_node
        | Kind.Uninitialized -> ()
        | Kind.UnorderedArrayFold fold ->
            { new UnorderedArrayFoldEval<_, _> with
                member _.Eval fold =
                    UnorderedArrayFold.invariant ignore inv fold |> FakeUnit.ofUnit
            }
            |> fold.Apply
            |> FakeUnit.toUnit
