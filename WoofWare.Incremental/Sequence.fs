...
module private SequenceUnfoldCrate =
    let make init f =
        { new SequenceUnfoldCrate<_> with member _.Apply e = e.Eval init f }
...
module Sequence =
    ...
    let ofList<'a> (l : 'a list) : 'a Sequence =
        Sequence.OfList l
            ...
            { new SequenceUnfoldEval<_, _> with
                member _.Eval init f =
                    ...
                    | None ->
                        ...
                        None
                    | Some (result, init) ->
                        (result, Sequence.Unfold (SequenceUnfoldCrate.make init f))
                        |> Some
            }
            |> c.Apply
        ...
        | Sequence.Unfold cr ->
            ...
            { new SequenceUnfoldEval<_, _> with
                member _.Eval<'state> (origInit : 'state) (origF : 'state -> ('a * 'state) option) : 'acc =
                    ...
                    while (next <- origF unfoldState; next.IsSome) do
    ...
    let head (s : 'a Sequence) : 'a option =
        next s |> Option.map fst
